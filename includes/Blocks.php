...
class Blocks {
	...
	public function do_reusable_block( $attributes ) {
		...
		if ( ! $form || 'formiflex_form' !== $form->post_type ) {
			return '';
		}
		...
		wp_interactivity_config(
			...
			array(
				...
				'nonce' => wp_create_nonce( '_formiflex' ),
				'settings' => \Formiflex\Utils\formiflex_frontend_options(),
			)
		);

		$form_context = \Formiflex\Utils\formiflex_form_context( $attributes['ref'] );
		...
		if ( $p->next_tag( 'form' ) ) {
			$p->set_attribute( 'data-wp-interactive', 'formiflex' );
			...
			$p->set_attribute( 'id', 'formiflex-' . $attributes['ref'] );
		}

		if ( $p->next_tag(
			array(
				'tag_name' => 'input',
				'class_name' => 'formiflex-hp',
			)
		) ) {
			$p->set_attribute( 'name', '_formiflex_h' . $attributes['ref'] );
			$p->set_attribute( 'aria-label', __( 'If you are human, leave this field blank.', 'formiflex' ) );
		}
		...
		if ( ! is_admin() && current_user_can( 'manage_options' ) && $form_context['debug'] ) {
			...
			$debug = '<div data-wp-interactive="formiflex" data-wp-bind--hidden="!state.debugData">
			<p>Debug output</p>
	...
	/**
	 * Add formiflex block category
	 *
	 * @param  array $categories The categories of Gutenberg.
	 */
	public function register_block_category( $categories ) {
		$currentScreen = get_current_screen();
		if ( 'formiflex' === $currentScreen->id ) {
			return array_merge(
				array(
					array(
						'slug'  => 'formiflex',
						'title' => __( 'Formiflex', 'formiflex' ),
					),
				),
				$categories
			);
	...
	/**
	 * Add formiflex block pattern category
	 */
