...
class Frontend {
	...
	public function listen_for_submit() {
		...
		if ( ! check_ajax_referer( '_formiflex', '_formiflex', false ) ) {
			wp_send_json_error( array( 'message' => __( 'Wrong nonce', 'formiflex' ) ), 500 );
			wp_die();
		}

		if ( empty( $_POST['_formiflex_id'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Missing ID', 'formiflex' ) ), 500 );
			wp_die();
		}

		$form_id = absint( $_POST['_formiflex_id'] );
		$form    = new \Formiflex\Processor\Form( $form_id );
		$form->validate();
	...
	public function process_form( $form ) {
		...
		*/
		do_action( 'formiflex_process_form', $form );
			...
			foreach ( $actions as $action_settings ) {
				...
				if ( $action_settings['async'] && ! $form->is_debug() ) {
					wp_schedule_single_event(
						time() + 60,
						'formiflex_process_form_action_' . $action_settings['type'],
						array(
		 ...
		 * @param Form $form
		 */
		do_action( "formiflex_form_{$form->get_id()}_success", $form );
		 ...
		 * @param Form $form
		 */
		do_action( 'formiflex_form_success', $form );
	}
}
