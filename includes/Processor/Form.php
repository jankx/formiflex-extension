...
namespace Formiflex\Processor;

use function Formiflex\Utils\formiflex_default_options as default_options;
...
class Form {
	...
	public function __construct( $id ) {
		if ( ! empty( $id ) ) {
			...
			$this->actions  = get_post_meta( $id, '_formiflex_actions', true );
		}
	}
	...
	private function get_settings( $id ) {
		...
		return wp_parse_args( get_post_meta( $id, '_formiflex_settings', true ), $default );
	}
	...
	public function validate() {
		$v = new \Formiflex\Processor\Validator( $this->ID, $this->settings );
		$v->validate();
	...
	public function save() {
		...
		$values = array();
		$place_holders = array();
		$submissions_table = $wpdb->prefix . 'formiflex_submissions';
		$submissions_meta_table = $wpdb->prefix . 'formiflex_submissions_meta';
		$form_id = $this->ID;
		$data  = array(
		...
		$query = 'INSERT INTO ' . $wpdb->prefix . 'formiflex_submissions_meta (form_id, submission_id, field_name, field_value ) VALUES ';
		$query .= implode( ', ', $place_holders );

		//phpcs:ignore
		$wpdb->query( $wpdb->prepare( $query, $values ) );

		$formiflex_result = get_transient( 'formiflex_news', false );
		set_transient( 'formiflex_news', $formiflex_result + 1, DAY_IN_SECONDS );
	}
	...
	public function get_response() {
		...
		return apply_filters( 'formiflex_form_response', $response );
	}
	...
	private function replace_tags( $template ) {
		$replacer = new \Formiflex\TagReplacers\Replacer( $this->data['fields'] );
	...
	private function get_message( $code ) {
		...
		$message = apply_filters( 'formiflex_form_message_' . $code, $message );
		return $message;
	}
}
