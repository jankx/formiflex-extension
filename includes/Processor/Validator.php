...
use Formiflex\TagReplacers\Replacer;
...
class Validator {
	...
	private function is_spam() {
		...
		if ( empty( $this->id ) ) {
			$this->add_error( __( 'Probably a spam request.', 'formiflex' ) );
			return true;
		}

		if ( ! check_ajax_referer( '_formiflex', '_formiflex', false ) ) {
			$this->add_error( __( 'Invalid ajax referer.', 'formiflex' ) );
			return true;
		}
		...
		$honeypot_key = sprintf( '_formiflex_h%d', $this->id );
	...
	private function sanitize() {
		 ...
		 */
		$this->fields = apply_filters( 'formiflex_data_fields', $this->fields, $this->settings['fields'] );
	}
	...
	private function sanitize_field( $key, $value ) {
		...
		if ( 'checkbox' === $fields[ $key ] ) {
			...
			return $value ? $value : __( 'yes', 'formiflex' );
		}
	...
	private function validate_fields() {
		...
		if ( ! empty( $this->settings['fields'] ) ) {
			...
			$validator->addValidator( 'maxlength', new \Formiflex\Validators\MaxLengthRule() );
			$validator->addValidator( 'minlength', new \Formiflex\Validators\MinLengthRule() );
		 ...
		 * @param array $data
		 */
		$errors = apply_filters( 'formiflex_validate_form_' . $this->id, $errors, $this->fields, $this );
		 ...
		 *
		 * @param string $errors
		 * @param Formiflex\Validator $this
		 */
		$errors = apply_filters( 'formiflex_validate_form', $errors, $this->fields, $this );
	...
	private function validate_recaptcha() {
		...
		if ( empty( $this->params['g-recaptcha-response'] ) ) {
			$this->add_error( __( 'Invalid reCaptcha.', 'formiflex' ) );
			return false;
		}
	...
	private function validate_hcaptcha() {
		...
		if ( empty( $this->params['h-captcha-response'] ) ) {
			$this->add_error( __( 'Invalid captcha.', 'formiflex' ) );
			return false;
		}
