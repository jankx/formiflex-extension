...
class Importer extends Base {
	...
	public function import_forms( $request ) {
		...
		if ( $fileSize > 3145728 ) { // 3 MB (1 byte * 1024 * 1024 * 3 (for 3 MB)).
			return $this->error( 'file_too_large', __( 'File max size is 3MB.', 'formiflex' ) );
		}
		...
		if ( ! in_array( $filetype, array_keys( $allowedTypes ) ) ) {
			return $this->error( 'wrong_filetype', __( 'Allowed file is json.', 'formiflex' ) );
		}
		...
		foreach ( $forms as $form ) {
			...
			$my_post = array(
				...
				'post_type'    => $form->type,
				'meta_input' => array(
					'_formiflex_actions' => $form->meta->_formiflex_actions,
					'_formiflex_settings' => $form->meta->_formiflex_settings,
				),
			);
		...
		return array(
			'success' => true,
			'response' => __( 'Forms imported!', 'formiflex' ),
		);
	}
