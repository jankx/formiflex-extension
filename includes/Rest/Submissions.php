...
class Submissions extends Base {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'id' => array(
							'description' => __( 'Unique identifier for the submission.', 'formiflex' ),
							'type'        => 'integer',
							'required'    => true,
						),
					),
	...
	public function get_submissions( $request ) {
		...
		$params = array(
			'column'   => $wpdb->prefix . 'formiflex_submissions',
			'form_id'  => $form_id,
			'search'   => '',
			'order_by' => 'id',
			'order'    => 'DESC',
	...
	public function edit_submission( $request ) {
		...
		global $wpdb;
		$submission_table = $wpdb->prefix . 'formiflex_submissions';
	...
	public function delete_submission( $request ) {
		...
		$results = $wpdb->get_col(
			$wpdb->prepare(
				"DELETE from {$wpdb->prefix}formiflex_submissions where id = %d;",
				array( 'id' => $id )
			)
		);
	...
	private function get_columns( $form_id ) {
		...
		$results = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT DISTINCT field_name from {$wpdb->prefix}formiflex_submissions_meta where form_id = %d;",
	...
	private function get_from_db( $id ) {
		...
		$submission = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}formiflex_submissions s WHERE s.id = %d;",
				array( $id )
			),
			ARRAY_A
		);
