...
class Formatter {
	...
	public static function format( $value, $column_type, $limit = 150 ) {
		...
		switch ( $column_type ) {
			...
			case 'password':
				...
				break;
			default:
				$value = self::formiflex_field_value( $value, $limit );
		}
	 ...
	 * @since 1.0.0
	 */
	private static function formiflex_field_value( $value, $limit = 150, $escape_function = 'wp_kses_post' ) {
		if ( '' === $value ) {
			return $value;
		}

		if ( self::formiflex_is_file( $value ) ) {
		...
		if ( self::formiflex_is_date( $value ) ) {
			$date_format = get_option( 'date_format' );
	 ...
	 * @return bool
	 */
	private static function formiflex_is_file( $value ) {
	 ...
	 * @since 1.0.0
	 */
	private static function formiflex_is_date( $value ) {
		if ( ! is_string( $value )
				|| strlen( $value ) !== 10
	 ...
	 * @return string
	 */
	private static function formiflex_human_filesize( $size, $precision = 2 ) {
	    // phpcs:ignore
