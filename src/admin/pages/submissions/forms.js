					...
					return (
						<HStack expanded={ false } spacing="0">
							<Button
								...
								icon={ commentContent }
								iconSize="20"
								label={ __( 'View submissions', 'formiflex' ) }
								showTooltip
							></Button>
			...
			},
			{
				...
				render: ( { item } ) => {
					return (
						...
						>
							{ item.meta._formiflex_settings.storeSubmissions && (
								<Icon icon={ icons.Database } width={ 18 } />
							) }

							{ item.meta._formiflex_actions.map( ( a, i ) => (
								<Icon
									key={ i }
									width={ 18 }
									icon={ icons[ a.name ] }
		...
		<Fragment>
			<Header title={ __( 'Forms' ) } className="full-width">
				<Button
					variant="primary"
					size="small"
					href={ 'post-new.php?post_type=formiflex_form' }
				>
					{ __( 'Add new' ) }
				</Button>
			</Header>
