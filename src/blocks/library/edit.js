...
export default function ReusableBlockEdit( {
	...
	const options = useSelect( ( select ) => {
		...
		const opts = [
			{ value: '', label: __( 'Select a form', 'formiflex' ) },
		];

		forms?.forEach( ( post ) => {
			opts.push( {
				value: post.id,
				label: post.title.raw || __( 'No title', 'formiflex' ),
			} );
		} );
	...
	const create = useCallback(
		async ( title = null ) => {
			const template = {
				title,
				content: `<!-- wp:formiflex/form {"lock":{"move":false,"remove":true}} -->
				<form method="post" class="wp-block-formiflex-form" novalidate>
				<input type="hidden" name="_formiflex_id"/>
				<input type="text" class="formiflex-hp" tabindex="-1"/>
	...
	const [ blocks ] = useEntityBlockEditor( 'postType', 'formiflex_form', {
		id: ref,
	} );
	...
	if ( ! ref || isMissing ) {
		return (
			<div { ...blockProps }>
				<InspectorControls>
					<PanelBody
						title={ __( 'Form Settings', 'formiflex' ) }
						initialOpen={ true }
					>
						<SelectControl
							label={ __( 'Choose a form', 'formiflex' ) }
							options={ options }
							onChange={ ( val ) => {
				...
				<Placeholder
					...
					label={ __( 'Insert a form', 'formiflex' ) }
				>
					{ 'widgets' === window.pagenow ? (
						...
						<VStack style={ { width: '100%' } }>
							...
							<HStack alignment={ 'left' }>
								...
								>
									{ __( 'Create a new form', 'formiflex' ) }
								</Button>
							</HStack>
						</VStack>
					) }
