...
const toggleInputError = () => {
	...
	if (
		'tel' === ref.type &&
		ref.classList.contains( 'formiflex-advanced' ) &&
	...
	if (
		...
		const checked = ref
			.closest( '.wp-block-formiflex-multichoices' )
		...
		if ( checked ) {
			...
			ref.closest( '.wp-block-formiflex-multichoices' )
...
const formSubmit = async () => {
	...
	formData.append( 'action', 'formiflex' );
	formData.append( '_formiflex', config.nonce );
	formData.append( '_formiflex_id', id );
...
const response = ( ref, res, state ) => {
	...
	if ( data.hide && res.success ) {
		const msg = ref.querySelector( '.wp-block-formiflex-message' );
...
const { state } = store( 'formiflex', {
	...
	callbacks: {
		...
		init: () => {
			...
			document
				.querySelectorAll( 'input.formiflex-advanced[type=text]' )
				.forEach( ( el ) => {
			...
			document
				.querySelectorAll( 'input[type="tel"].formiflex-advanced' )
				.forEach( ( el ) => {
			...
			document
				.querySelectorAll( 'select.formiflex-advanced' )
				.forEach( ( el ) => {
