...
function AdvancedOptions( props ) {
	...
	const addOutput = () => {
		let child = 'formiflex/button';
		if ( 'range' === type ) {
			child = 'formiflex/output';
		}
		replaceInnerBlocks(
			props.clientId,
	...
	return (
		<Fragment>
			{ supported.includes( 'autocomplete' ) && (
				<ToggleControl
					label={ __( 'Autocomplete', 'formiflex' ) }
					checked={ enableAutoComplete }
					onChange={ ( newval ) =>
						setAttributes( { enableAutoComplete: newval } )
					}
					help={ __( 'Hint for form autofill feature.', 'formiflex' ) }
					__nextHasNoMarginBottom
				/>
			) }
			{ enableAutoComplete && supported.includes( 'autocomplete' ) && (
				<TextControl
					label={ __( 'Autocomplete attribute', 'formiflex' ) }
					value={ autocomplete }
					onChange={ ( newval ) =>
			...
			{ 'range' === type && (
				<ToggleControl
					label={ __( 'Show output', 'formiflex' ) }
			...
			{ 'hidden' !== type && (
				<Fragment>
					...
					/>
					<ToggleControl
						label={ __( 'Read only', 'formiflex' ) }
						checked={ readonly }
						onChange={ ( newval ) =>
							setAttributes( { readonly: newval } )
						}
						help={ __( 'Make value not editable.', 'formiflex' ) }
						__nextHasNoMarginBottom
					/>
				</Fragment>
			) }
...
export default withFilters( 'formiflex.advancedOptions' )( AdvancedOptions );
