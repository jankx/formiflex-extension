...
export function ActionsModal( props ) {
	...
	const save = ( item ) => {
		if ( ! item.id ) {
			const newItem = Object.assign( item, {
				id: meta._formiflex_actions.length + 1,
			} );
			const actions = [ ...meta._formiflex_actions, newItem ];
			setActions( actions );
		} else {
			const actions = meta._formiflex_actions.map( ( el ) =>
	...
	const remove = ( item ) => {
		const actions = meta._formiflex_actions.filter( ( o ) => {
			return o.id !== item.id;
		} );
		setActions( actions );
		onRequestClose( false );
	...
	return (
		<Modal
			...
			size="large"
		>
			<div className="formiflex-action-modal">
				{ 'email' !== settings.type && (
					<Promo { ...props } mergeTags={ MergeTags } />
				) }
				<Fragment>
					{ applyFilters(
						'formiflex.modal.' + settings.type,
						'',
						clientId,
						action,
						updateSettings,
					...
					<div className="formiflex-modal-buttons">
						<Button
							variant="primary"
							onClick={ () => {
								save( action );
