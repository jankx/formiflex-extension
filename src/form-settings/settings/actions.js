...
export function ActionsSettings() {
	...
	if ( ! meta._formiflex_actions ) {
		return <></>;
	}

	const remove = ( item ) => {
		const arr = meta._formiflex_actions.filter( ( o ) => {
			return o.id !== item.id;
		} );
		setMeta( { ...meta, _formiflex_actions: arr } );
		setShowModal( false );
	};
	...
	return (
		<Fragment>
			...
			/>
			<VStack className={ 'formiflex-actions-list' }>
				{ meta._formiflex_actions.map( ( a, i ) => {
					return (
						<HStack
							key={ i }
							direction="row"
							className={ 'formiflex-action-item' }
						>
